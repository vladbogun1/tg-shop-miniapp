...
function renderProducts(){
  ...
  for (const p of state.products) {
    ...
    const btnRow = el("div", { class:"actions" }, [
      el("button", { class:"pill", onclick: (e)=>{ e.stopPropagation(); openProduct(p); }}, [document.createTextNode("Подробнее")]),
      el("button", { class:"primary pill js-add", onclick: (e)=>{ e.stopPropagation(); addToCart(p.id, 1); }}, [document.createTextNode("В корзину")]),
    ]);
...
function openCart(){
  ...
  for (const [id, it] of state.cart.entries()) {
    ...
    lines.push(el("div", { class:"cart-line" }, [
      ...
      el("div", { class:"qty" }, [
        el("button", { onclick: ()=> { addToCart(id, -1); openCart(); } }, [document.createTextNode("−")]),
...
function openCheckout(){
  ...
  form.append(
    ...
    el("label", {}, [document.createTextNode("Имя"), el("input", { name:"customerName", required:"true", autocomplete:"name" })]),
    el("label", {}, [document.createTextNode("Телефон"), el("input", { name:"phone", required:"true", autocomplete:"tel" })]),
    el("label", {}, [document.createTextNode("Адрес доставки"), el("textarea", { name:"address", required:"true", rows:"3", autocomplete:"street-address" })]),
    el("label", {}, [document.createTextNode("Комментарий (опционально)"), el("textarea", { name:"comment", rows:"2" })]),
    el("div", { class:"hr" }),
    el("div", { class:"row" }, [
