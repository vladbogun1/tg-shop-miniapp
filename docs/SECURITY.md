...
## 7. Известный остаток (требует решения владельца)

**Платёжные реквизиты в истории git.** Миграция `V2__seed.sql` содержит реальный номер карты,
IBAN, ФИО и РНОКПП. Исправить файл нельзя: миграция уже накатана, Flyway сверяет контрольную
сумму и упадёт на всех существующих БД; затирать данные новой миграцией тоже нельзя — на проде
это живые реквизиты, по которым платят.

Данные уже в истории коммитов, поэтому правка файла в `HEAD` ничего не удалит. Реальные варианты:

1. переписать историю (`git filter-repo` + force-push) — ломает все клоны, решает владелец;
2. считать карту скомпрометированной и выпустить новую, задав реквизиты через админку
   (Админка → «Оплата» → «Реквизиты»);
3. если репозиторий приватный и останется приватным — принять риск осознанно.

Новые установки реквизиты через сид больше не получают только после варианта (1);
до этого момента их надо перезаписать через админку сразу после первого запуска.
