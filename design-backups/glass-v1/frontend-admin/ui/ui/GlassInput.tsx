 ...
 * No browser outline; glass container; accent focus ring; danger/ok border.
 */
      ...
      >
        <label
          ...
          className={`pointer-events-none absolute left-4 origin-left text-[var(--text-faint)] transition-all duration-200 ${
            floated
              ? "top-1.5 text-[11px] text-[var(--text-muted)]"
        ...
        <input
          ...
          className="w-full bg-transparent pb-2 pt-5 text-[15px] text-[var(--text)] outline-none placeholder:text-transparent"
          {...rest}
        />
      </div>
      {hint && (
        <p
          className={`mt-1 px-1 text-[12px] ${
            status === "danger" ? "text-[var(--danger)]" : "text-[var(--text-faint)]"
          }`}
        >
          {hint}
        </p>
